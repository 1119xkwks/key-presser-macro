...
export const SHORTCUT_KEYS: KeyOption[] = [
    // TARGET_KEYS의 기본 키들을 Electron 형식으로 변환하여 포함
    ...TARGET_KEYS.map(key => ({
        label: key.label,
        // PowerShell 형식({F1})을 Electron 형식(F1)으로 변환
        value: key.value.replace(/[{}]/g, '').replace('BS', 'Backspace').replace('INS', 'Insert').replace('DEL', 'Delete').replace('PGUP', 'PageUp').replace('PGDN', 'PageDown').replace('PRTSC', 'PrintScreen').replace('UP', 'Up').replace('DOWN', 'Down').replace('LEFT', 'Left').replace('RIGHT', 'Right')
    })),
    // 조합키 (Ctrl+)
